...
struct Dialog {
    ...
    void create() {
        ...
        control(0,L"STATIC",L"仅使用最优码组句的高频字数量",0,18,172,390,28);
        control(SentenceCommon,L"EDIT",std::to_wstring(initialSentence.commonCharacterLimit).c_str(),ES_NUMBER|ES_AUTOHSCROLL|WS_TABSTOP,430,168,155,28);
        control(0,L"STATIC",L"默认 1500；设为 0 时不按高频字限制全码。",0,18,206,580,24,true);
        control(0,L"STATIC",L"允许全码组句的例外字符（可留空）",0,18,238,580,28);
        control(SentenceWhitelist,L"EDIT",wide(initialSentence.fullCodeWhitelist.c_str()),ES_AUTOHSCROLL|WS_TABSTOP,18,272,567,30);
        SendMessageW(item(SentenceWhitelist),EM_SETLIMITTEXT,4*1024*1024,0);
        control(0,L"STATIC",L"直接填写汉字，无需分隔；这些字不受上方的最优码限制。",0,18,310,580,24,true);
        control(SentenceLearning,L"BUTTON",L"Tab 锁重上屏后自学习（仅本方案、本地保存）",BS_AUTOCHECKBOX|WS_TABSTOP,18,342,580,28);
