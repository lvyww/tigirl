...
[[noreturn]] void invalid(){throw std::runtime_error("Invalid TCSKNM03 fivegram model");}
...
struct SentenceFivegram::Data {
    ...
    void validate(){
        ...
        if(read<std::uint32_t>(8)!=1||read<std::uint32_t>(12)!=headerSize||read<std::uint64_t>(16)!=length||read<std::uint32_t>(24)!=5)invalid();
        ...
        for(int i=0;i<5;++i){const auto at=160+static_cast<std::uint64_t>(i)*16;quant[i].pmin=read<std::int32_t>(at)/1e7;quant[i].pstep=read<std::uint32_t>(at+4)/1e12;quant[i].bmin=read<std::int32_t>(at+8)/1e7;quant[i].bstep=read<std::uint32_t>(at+12)/1e12;if(!std::isfinite(quant[i].pmin)||!std::isfinite(quant[i].pstep)||!std::isfinite(quant[i].bmin)||!std::isfinite(quant[i].bstep))invalid();}
        unigrams.resize(vocabCount);tokenIds.reserve(vocabCount*2);std::uint64_t position=vocabOffset,end=vocabOffset+vocabBytes;
        for(std::uint32_t id=0;id<vocabCount;++id){if(position>end||6>end-position)invalid();const auto size=read<std::uint16_t>(position);position+=2;if(size>end-position||4>end-position-size)invalid();std::string_view token(reinterpret_cast<const char*>(data+position),size);position+=size;const auto pq=read<std::uint16_t>(position);position+=2;read<std::uint16_t>(position);position+=2;unigrams[id]=probability(1,pq);if(!tokenIds.emplace(token,static_cast<std::uint16_t>(id)).second)invalid();}
    ...
    Lookup lookup(int order,const std::array<std::uint16_t,4>& context,std::uint16_t target) const {
        ...
        const auto blockHeader=static_cast<std::uint64_t>(contextLength)*2+4;for(std::uint32_t n=0;n<indexStride&&position<pageEnd;++n){if(blockHeader>pageEnd-position)invalid();std::array<std::uint16_t,4> key{};for(int i=0;i<contextLength;++i)key[i]=read<std::uint16_t>(position+i*2);const auto compared=compareIds(key.data(),context.data(),contextLength);const auto bq=read<std::uint16_t>(position+contextLength*2);const std::uint32_t count=read<std::uint16_t>(position+contextLength*2+2);const auto successors=position+blockHeader;const auto successorBytes=static_cast<std::uint64_t>(count)*4;if(successorBytes>pageEnd-successors)invalid();if(compared==0){std::uint32_t left=0,right=count;while(left<right){const auto mid=left+(right-left)/2;const auto id=read<std::uint16_t>(successors+static_cast<std::uint64_t>(mid)*4);if(id<target)left=mid+1;else right=mid;}const auto bow=backoff(contextLength,bq);if(left<count){const auto at=successors+static_cast<std::uint64_t>(left)*4;if(read<std::uint16_t>(at)==target)return {probability(order,read<std::uint16_t>(at+2)),bow,true};}return {0,bow,false};}if(compared>0)return {};position=successors+successorBytes;}return {};
    }
