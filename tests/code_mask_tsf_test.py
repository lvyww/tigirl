...
try:
 for arch,machine in [('ARM64','arm64'),('x64','amd64'),('Win32','x86')]:
  with tempfile.TemporaryDirectory(prefix='mask-tsf-'+arch+'-',dir=ROOT/'build') as temp:
   root=Path(temp);(root/'.tsf-mask-test').touch();package=root/'package';package.mkdir()
   source=ROOT/'build/ARM64X/ARM64EC/Release';dll=ROOT/'build/Win32/Release/SampleIME.dll' if arch=='Win32' else source/'SampleIME.dll'
   shutil.copy2(dll,package/'SampleIME.dll');shutil.copy2(source/'tiger-v2.tcd',package/'tiger-v2.tcd')
   ...
   result=run_windows([str(exe),win(package/'SampleIME.dll'),'-',win(manifest),'--mask-detached',win(root)],capture_output=True,text=True,timeout=30)
