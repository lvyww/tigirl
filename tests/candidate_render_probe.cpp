...
int wmain(int argc,wchar_t** argv) {
    try {
        ...
        for(UINT dpi:{96u,120u,144u,192u}) {
            ...
            auto pos=candidatePosition(caret,work,w,h);
            require(pos.x+w<=work.right-2 && pos.y+h==work.bottom-2,"Work-area bottom/right clamp failed");
            const auto smaller=candidatePosition(caret,work,w,10);
            require(smaller.y==caret.bottom+5,"Shrink retained a stale above-caret anchor");
            caret={-1800,20,-1799,40};pos=candidatePosition(caret,work,w,h);
