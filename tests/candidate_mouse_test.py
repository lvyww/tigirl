...
try:
 for arch,machine in [('ARM64','arm64'),('x64','amd64'),('Win32','x86')]:
  with tempfile.TemporaryDirectory(prefix='candidate-mouse-'+arch+'-',dir=ROOT/'build') as temp:
   ...
   dll=ROOT/'build/Win32/Release/SampleIME.dll' if arch=='Win32' else source/'SampleIME.dll'
   for name in ['SampleIME.dll','tiger-v2.tcd','schema_manager.exe']:
    shutil.copy2(dll if name=='SampleIME.dll' else source/name,package/name)
   ...
   result=run_windows([str(exe),win(package/'SampleIME.dll'),win(root/'capture.bmp'),win(manifest),'--candidate-mouse',win(root)],capture_output=True,text=True,timeout=30)
 ...
 (ROOT/'build/candidate-mouse-validation.json').write_text(json.dumps(report,indent=2)+'\n')
