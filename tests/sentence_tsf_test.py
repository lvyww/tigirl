...
for arm64x in ([True] if os.environ.get('SENTENCE_ARM64X_ONLY') else [False]):
 if os.environ.get('SENTENCE_X86'):
  dll=Path(os.environ.get('SENTENCE_X86_PACKAGE_DIR',str(BUILD/'Win32/Release')))/'SampleIME.dll';manifest=dll.parent/'NativeTiger.x86.manifest'
 ...
 for platform,host,manifest in hosts:
  with tempfile.TemporaryDirectory(prefix='sentence-tsf-',dir=BUILD) as tmp:
   ...
   r=run_windows([BUILD/'tests/ARM64/lexicon_import.exe','--schema',win(source),win(root/'pinyin'),win(user),'测试整句','zh-CN'],capture_output=True,text=True,timeout=60);assert r.returncode==0,r.stderr
   if journal_mode:
    if bundled:
     assert (dll.parent/'lexicon_import.exe').read_bytes()==(BUILD/'tests/ARM64/lexicon_import.exe').read_bytes(),'Packaged importer is stale'
    else:shutil.copy2(BUILD/'tests/ARM64/lexicon_import.exe',dll.parent/'lexicon_import.exe')
    ...
    revision=run_windows([BUILD/'tests/ARM64/lexicon_import.exe','--sentence-revision',win(base),win(journal)],capture_output=True,text=True,timeout=60)
    ...
    updated=run_windows([BUILD/'tests/ARM64/lexicon_import.exe','--update',win(source),win(root/'pinyin'),win(user),'测试整句','zh-CN'],capture_output=True,text=True,timeout=60)
    ...
    imported=run_windows([BUILD/'tests/ARM64/lexicon_import.exe','--schema',win(other),win(root/'pinyin'),win(user),'另一整句','zh-CN'],capture_output=True,text=True,timeout=60)
