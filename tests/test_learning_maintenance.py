...
class MaintenanceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.path = self.root / '.tigirl-learning.tsv'
    ...
    def test_hand_edit_without_checksum(self):
        text=self.path.read_text(encoding='utf-8').replace('虎𰻞','虎娘').rstrip('\n')
        self.path.write_text('\ufeff# 用户编辑\n'+text,encoding='utf-8')
        self.assertTrue(all(e['text']=='虎娘' for e in m.parse(self.path.read_bytes())[0]))
        m.maintain(self.path,'compact')
        self.assertIn('虎娘',self.path.read_text(encoding='utf-8'))
    ...
    def test_import_idempotent(self):
        source=self.root/'export.json'; source.write_text(json.dumps({'format':'TCL2','events':self.events}),encoding='utf-8')
    ...
    def test_import_all_or_nothing(self):
        old=self.path.read_bytes(); source=self.root/'bad.json'; source.write_text(json.dumps({'format':'TCL2','events':[dict(self.events[0], id='new'),{}]}),encoding='utf-8')
    ...
    def test_compact_torn_tail_and_corruption(self):
        ...
        with self.assertRaises(ValueError): m.maintain(self.path,'compact')
        self.assertEqual(self.path.read_bytes(),before)
    def test_reject_code_table_name(self):
        for name in ['词库.txt','.tigirl-learning.tsv.bak.txt','scheme.dict.yaml']:
