...
int OpenReadOrThrow(const char *name) {
  ...
  // TigerClaw passes UTF-8 paths; the Windows ANSI code page cannot represent
  // all user/profile directories. Keep the model mapped read-only.
  int length = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, NULL, 0);
  UTIL_THROW_IF(length == 0, ErrnoException, "invalid UTF-8 model path");
  std::wstring wide(length, L'\0');
  MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, &wide[0], length);
  UTIL_THROW_IF(-1 == (ret = _wopen(wide.c_str(), _O_BINARY | _O_RDONLY)), ErrnoException, "while opening " << name);
#else
